...
def test_array_array_array()
{
    ...
    os.print("TEST ARRAY ARRAY ARRAY BEGIN")
    ...
    for (i = 0; i < 3; i = i + 1){
        for (j = 0; j < 3; j = j + 1){
            for (k = 0; k < 3; k = k + 1){
                ...
                os.assert(safe_3d_array[i][j][k] == check, "safe_3d_array[i][j][k] == check")
            }
        }
    }

    os.print("TEST ARRAY ARRAY ARRAY SUCCESS")
}
...
#script("script/test_all_one/basic/variable/array/test_array_array_array.ll")
