...
def test_hash_tuple1()
{
    ...
    for (i = 0; i < value.size(); i = i + 1) {
        os.assert(value{i} == compare_safe_tuple{i}, "value{i} == compare_safe_tuple{i}")
    }

    os.assert(safe_tuple.size() == 0, "safe_tuple.size() is 0")

    safe_hash.add_key_item(safe_1, safe_tuple2)
    value2 = safe_hash.find_item(safe_1)
    for (i = 0; i < value2.size(); i = i + 1) {
        os.assert(value2{i} == compare_safe_tuple2{i}, "value2{i} == compare_safe_tuple2{i}")
    }
    ...
    for (i = 0; i < value3.size(); i = i + 1) {
        os.assert(i == 0, "i == 0")
        os.assert(value3{i} == "hello world value3", "hello world value3")
    }
}
...
def test_hash_tuple_arg(var safe_hash)
{
    ...
    value3.add_item("hello world value3")
    for (i = 0; i < value3.size(); i = i + 1) {
        os.assert(value3{i} == compare_safe_tuple{i}, "value3{i} == compare_safe_tuple{i}")
    }
}
...
def test_hash_tuple_arg2(var safe_tuple_arg)
{
    ...
    for (i = 0; i < safe_tuple_arg.size(); i = i + 1) {
        os.assert(safe_tuple_arg{i} == compare_safe_tuple{i}, "safe_tuple_arg{i} == compare_safe_tuple{i}")
    }
}
...
def test_hash_tuple3()
{
    ...
    safe_tuple.add_item("hello world")
    os.assert(safe_tuple.size() == 2, "safe_tuple.size() == 2")
    os.assert(safe_tuple{0} == "hello world", "safe_tuple{0} == hello world")
    os.assert(safe_tuple{1} == "hello world", "safe_tuple{1} == hello world")
}

def test_hash_tuple()
{
    os.print("TEST HASH TUPLE BEGIN")
    test_hash_tuple1()
    test_hash_tuple2()
    test_hash_tuple3()
    os.print("TEST HASH TUPLE SUCCESS")
}
...
#script("script/test_all_one/basic/variable/hash/test_hash_tuple.ll")
