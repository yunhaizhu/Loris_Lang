...
def test_tuple_arg(var safe_tuple)
{
    ...
    for (i = 0; i < compared_array.size(); i+=1){
        assert(safe_tuple{i} == compared_array[i], "8 safe_tuple{i} == compared_array[i]")
    }

    safe_tuple.del_item_idx(8)
    for (i = 0; i < compared_array2.size(); i+=1){
        assert(safe_tuple{i} == compared_array2[i], "8 safe_tuple{i} == compared_array2[i]")
    }

    safe_tuple.del_item(8)
    for (i = 0; i < compared_array3.size(); i+=1){
        assert(safe_tuple{i} == compared_array3[i], "8 safe_tuple{i} == compared_array3[i]")
    }
}
def test_tuple8()
{
