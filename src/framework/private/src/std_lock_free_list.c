 ...
 * @return  std_rv_t
 */
std_rv_t std_lock_free_list_init(INOUT std_lock_free_list_head_t *head, IN std_uint_t offset, IN std_bool_t free_enable, IN std_u8_t key_enable)
{
...
std_rv_t std_lock_free_list_add(IN std_lock_free_list_head_t *head, IN std_lock_free_list_head_t *entry, IN std_u64_t u64_key)
{
    ...
    while (result != STD_RV_SUC) {
        if (std_lock_free_list_inline_find(head, u64_key, head->offset, &prev, &cur, &next) == STD_BOOL_TRUE) {
            if (head->key_enable == KEY_DISABLE){
                std_lock_free_list_next(head, &prev);
                cur = prev->next;
            }else if (head->key_enable == KEY_ENABLE_NODUPLICATE){
...
std_bool_t std_lock_free_list_find_index(IN std_lock_free_list_head_t *head, std_int_t index,
                                         INOUT std_lock_free_list_head_t **prev_entry,
                                         INOUT std_lock_free_list_head_t **curr_entry,
                                         INOUT std_lock_free_list_head_t **next_entry)
{
    ...
    std_lock_free_list_head_t *entry;
    std_hazard_ptr_t *hp0 = std_hazard_get((std_hazard_head_t *) head->hazard_head, 0);
    std_lock_free_list_head_t *prev;

    STD_ASSERT_RV(head != NULL, STD_BOOL_FALSE);
    ...
    while (entry) {
        ...
        if (count == index){
            *prev_entry = prev;
            *curr_entry = entry;
            *next_entry = STD_LOCK_FREE_LIST_STRIP_MARK(entry->next);
            break;
        }
...
std_rv_t std_lock_free_list_add_index(IN std_lock_free_list_head_t *head, IN std_int_t index, IN std_lock_free_list_head_t *entry, IN std_u64_t u64_key)
{
    ...
    while (result != STD_RV_SUC) {
        if (std_lock_free_list_inline_find(head, u64_key, head->offset, &prev, &cur, &next) == STD_BOOL_TRUE) {
            if (head->key_enable == KEY_DISABLE){
                std_lock_free_list_find_index(head, index, &prev, &cur, &next);
            }else if (head->key_enable == KEY_ENABLE_NODUPLICATE){
                result = STD_RV_ERR_FAIL;
                break;
            }
...
/**
 * std_lock_free_list_del_index_flag
 * @brief
 * @param   head
 * @param   u64_key
 * @param   del_flag
 * @return  STD_CALL std_rv_t
 */
STD_CALL std_rv_t std_lock_free_list_del_index_flag(IN std_lock_free_list_head_t *head, IN std_int_t index, IN std_bool_t del_flag)
{
    std_lock_free_list_head_t *prev = NULL;
    std_lock_free_list_head_t *cur = NULL;
    std_lock_free_list_head_t *next = NULL;
    ...
    while (result != STD_RV_SUC) {
        if (std_lock_free_list_find_index(head, index, &prev, &cur, &next) == STD_BOOL_FALSE) {
            result = STD_RV_ERR_FAIL;
            break;
        }
