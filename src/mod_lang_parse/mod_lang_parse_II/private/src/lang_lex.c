 ...
 */
STD_CALL std_int_t get_keyword_token(lang_state_t *state, IN const std_char_t *name, std_int_t name_len)
{
...
STD_CALL static inline std_int_t inline_handle_ID(lang_state_t *state)
{
    ...
    keyword_token = get_keyword_token(state, new_string, len);
    if (keyword_token) {
        FREE(new_string);
        new_string = NULL;
