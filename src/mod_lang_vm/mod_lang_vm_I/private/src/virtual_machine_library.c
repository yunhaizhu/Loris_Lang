...
STD_CALL std_void_t library_register(environment_vm_t *vm, std_int_t *register_id)
{
    ...
    for (std_int_t i = 1; i <= 20; ++i) {
        ...
        snprintf(name, sizeof(name), "eprint_%d", i);
        library_func_register(vm, &register_id[thread_id], name, i, library_eprint);
    }

    library_func_register(vm, &register_id[thread_id], "assert_2", 2, library_assert);

    library_func_register(vm, &register_id[thread_id], "random_number", 2, library_random_number);
    library_func_register(vm, &register_id[thread_id], "random_address", 1, library_random_address);
    library_func_register(vm, &register_id[thread_id], "random_string", 2, library_random_string);

    library_func_register(vm, &register_id[thread_id], "make_json", 2, library_make_json);
    library_func_register(vm, &register_id[thread_id], "parse_json", 2, library_parse_json);

    library_func_register(vm, &register_id[thread_id], "convert", 3, library_convert);

    library_func_register(vm, &register_id[thread_id], "check_type", 3, library_check_type);

    library_func_register(vm, &register_id[thread_id], "create_instance", 3, library_create_instance);
    library_func_register(vm, &register_id[thread_id], "delete_instance", 2, library_delete_instance);
    library_func_register(vm, &register_id[thread_id], "string_to_array", 2, library_string_to_array);
    library_func_register(vm, &register_id[thread_id], "array_to_string", 2, library_array_to_string);
    library_func_register(vm, &register_id[thread_id], "read_lines", 2, library_read_lines);
    library_func_register(vm, &register_id[thread_id], "get_hash_keys", 2, library_get_hash_keys);
}
