...
STD_CALL std_rv_t mod_lang_compile_I_init(IN mod_lang_compile_t *p_m, IN const std_char_t *arg, IN std_int_t arg_len)
{
    ...
    p_imp_m->p_compiled_body[get_std_thread_id()] = std_lock_free_key_hash_create(128);
...
{
    const mod_lang_compile_imp_t *p_imp_m = (mod_lang_compile_imp_t *) p_m;

    std_lock_free_key_hash_value_destroy(p_imp_m->p_compiled_body[get_std_thread_id()]);
...
STD_CALL std_char_t *mod_lang_compile_I_generated_bytecode(IN mod_lang_compile_t *p_m, IN std_char_t *file_name)
{
    ...
    compiled_body = std_lock_free_key_hash_find(p_imp_m->p_compiled_body[get_std_thread_id()], file_name, std_safe_strlen(file_name, KEY_NAME_SIZE));
    if (compiled_body == NULL) {
        compiled_body = (std_char_t *) CALLOC(1, MAX_CODE_SIZE);
        if (compiled_body == NULL) {
            STD_LOG(ERR, "Error allocating memory for compiled body\n");
            return NULL;
        }
        
        std_lock_free_key_hash_add(p_imp_m->p_compiled_body[get_std_thread_id()], file_name, std_safe_strlen(file_name, KEY_NAME_SIZE), compiled_body);
    } else {
        return compiled_body;
   }
    gen_buffer_output(compiled_body, MAX_CODE_SIZE);

    gen_buffer_reset();

    buf_len = std_safe_strlen(compiled_body, MAX_CODE_SIZE);
    if (buf_len >= MAX_CODE_SIZE - MAX_STRING_SIZE) {
        STD_LOG(ERR, "compiled body is too long, buf_len=%lu\n", buf_len);
        return NULL;
    }
    ...
    std_char_t body_file_name[KEY_NAME_SIZE] = "\0";
    std_safe_replace_chars(file_name, '/', '_');
    ...
    if (fp) {
        size_t bytes_written = fwrite(compiled_body, sizeof(char), buf_len, fp);
        if (bytes_written != buf_len) {
            STD_LOG(ERR, "Error writing compiled body to file %s\n", body_file_name);
        }
        fclose(fp);
    }
