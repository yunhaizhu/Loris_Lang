...
global_env_t global_env[THREAD_MAX];
 ...
 */
STD_CALL std_void_t add_require_func(const std_char_t *func_name)
{
    std_char_t *name = strdup(func_name);

    std_safe_strip_chars(name, '"');
    if (global_env[get_std_thread_id()].global_func_custom_extern_idx >= FUNC_EXTERN_EXTERN_LEN - 1){
        global_env[get_std_thread_id()].global_func_custom_extern_idx = 0;
    }
    global_env[get_std_thread_id()].global_func_custom_extern[global_env[get_std_thread_id()].global_func_custom_extern_idx++] = name;
}
...
{
    if (NULL == global_env[get_std_thread_id()].global_parse_error_need_clean_lang_ast_hash) {
        global_env[get_std_thread_id()].global_parse_error_need_clean_lang_ast_hash =
                std_lock_free_key_hash_create(KEY_NAME_SIZE);
    }
    std_char_t new_name[KEY_NAME_SIZE];
    snprintf(new_name, sizeof(new_name), "ast_%p", ast);
    std_lock_free_key_hash_add(global_env[get_std_thread_id()].global_parse_error_need_clean_lang_ast_hash, new_name, std_safe_strlen(new_name, sizeof(new_name)), ast);
}
 ...
 * @return  STD_CALL lang_ast_t *
 */
STD_CALL lang_ast_t *make_lang_ast_double(IN std_double_t number, IN std_char_t *source_file, IN std_int_t line)
{
    lang_ast_t *ast = NULL;
 ...
 * @return  STD_CALL lang_ast_t *
 */
STD_CALL lang_ast_t *make_lang_ast_address(IN std_void_t *address, IN const std_char_t *source_file, IN std_int_t line)
{
    lang_ast_t *ast = NULL;
 ...
 * @return  STD_CALL lang_ast_t *
 */
STD_CALL lang_ast_t *make_lang_ast_char(IN std_char_t chr, IN const std_char_t *source_file, IN std_int_t line)
{
    lang_ast_t *ast = NULL;
...
STD_CALL symbol_t *lookup_lang_ast_symbol(IN std_char_t *name, IN std_bool_t check_error)
{
    ...
    for (std_int_t i = 0; i < global_env[get_std_thread_id()].global_func_extern_idx; ++i) {
        if (0 == strcmp(name, global_env[get_std_thread_id()].global_func_extern[i])) {
    ...
    for (std_int_t i = 0; i < global_env[get_std_thread_id()].global_func_custom_extern_idx; ++i) {
        if (0 == strcmp(name, global_env[get_std_thread_id()].global_func_custom_extern[i]) && std_safe_strlen(name, KEY_NAME_SIZE) == std_safe_strlen(global_env[get_std_thread_id()].global_func_custom_extern[i], KEY_NAME_SIZE)) {
            extend_func = STD_BOOL_FALSE;
            global_env[get_std_thread_id()].create_id = STD_BOOL_TRUE;
    ...
    if (NULL == global_env[get_std_thread_id()].global_symbol_hash) {
        global_env[get_std_thread_id()].global_symbol_hash =
                std_lock_free_key_hash_create(KEY_NAME_SIZE);
    }

    if (global_env[get_std_thread_id()].function_name != NULL && extend_func) {
        snprintf(new_name, sizeof(new_name), "%s--%s", global_env[get_std_thread_id()].function_name, name);
    } else {
        if (global_env[get_std_thread_id()].create_func == STD_BOOL_TRUE) {
    ...
    symbol = std_lock_free_key_hash_find(global_env[get_std_thread_id()].global_symbol_hash, new_name, std_safe_strlen(new_name, sizeof(new_name)));
    if (symbol != NULL) {
        find = STD_BOOL_TRUE;
    }

    if (STD_BOOL_FALSE == find) {
        if (STD_BOOL_TRUE == global_env[get_std_thread_id()].create_id) {
            global_env[get_std_thread_id()].create_id = STD_BOOL_FALSE;

            symbol = (symbol_t *) CALLOC(sizeof(symbol_t), 1);
            symbol->name = strdup(new_name);
            std_lock_free_key_hash_add(global_env[get_std_thread_id()].global_symbol_hash, new_name, std_safe_strlen(new_name, sizeof(new_name)), symbol);
