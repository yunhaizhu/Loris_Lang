...
lang_ast_t *identifier(lang_state_t *state)
{
    ...
    if (state->lex_lookahead == TOKEN_ID) {
        ...
        if (lang_accept(state, ':')) {
            ...
            lang_expect(state, '}');
            ast->symbol->type_symbol = make_lang_ast(DECLARE_TUPLE_OP, lang_ast_type_symbol, make_lang_ast_list2(NULL, expr2, state->source_name, state->source_line), state->source_name, state->source_line);

            ast->symbol->check_block = fake_type_check_ast(global_env[get_std_thread_id()].function_name, check_symbol_name);
        }
...
lang_ast_t *local_var(lang_state_t *state)
{
    ...
    if (lang_accept(state, TOKEN_VAR)) {
        ...
        if (lang_accept(state, TOKEN_lang)) {
            ...
            lang_expect(state, '}');
            if (lang_accept(state, TOKEN_lang)) {
                return make_lang_ast(DECLARE_TUPLE_OP, symbol, make_lang_ast_list2(expr1, NULL, state->source_name, state->source_line), state->source_name, state->source_line);
            } else if (lang_accept(state, '=')) {
                lang_expect(state, '{');
                expr2 = vals_list(state);
                lang_expect(state, '}');
                lang_expect(state, TOKEN_lang);
                return make_lang_ast(DECLARE_TUPLE_OP, symbol, make_lang_ast_list2(expr1, expr2, state->source_name, state->source_line), state->source_name, state->source_line);
            }
        } else if (lang_accept(state, '<')) {
            lang_expect(state, '>');
            if (lang_accept(state, TOKEN_lang)) {
...
lang_ast_t *primary_expr(lang_state_t *state)
{
    ...
    if (lang_accept(state, TOKEN_ID)) {
        ...
        ast = make_lang_ast_number(0x7ff0000000000000|0x0003000000000000, state->source_name, state->source_line);
    }else if (lang_accept(state, '-')) {
        if (lang_accept(state, TOKEN_NUM)){
            ...
            ast = make_lang_ast_double(state->value.dou, state->source_name, state->source_line);
        }
    } else if (lang_accept(state, TOKEN_DOUBLE)) {
        ast = make_lang_ast_double(state->value.dou, state->source_name, state->source_line);
        ...
        ast = make_lang_ast_char((std_char_t)state->value.u64, state->source_name, state->source_line);
...
lang_ast_t *call_expr(lang_state_t *state)
{
    ...
    while (true) {
        if (lang_accept(state, '(')) {
            ...
            if (lang_accept(state, '=')) {
                expr3 = expression(state);
                ret = make_lang_ast(ADD_KEY_ITEM_OP, ret, make_lang_ast_list2(expr2, expr3, state->source_name, state->source_line), state->source_name, state->source_line);
            }else {
            ...
            if (lang_accept(state, TOKEN_ADD_ITEM)) {
                ...
                expr2 = arg_list(state);
                lang_expect(state, ')');
                ret = make_lang_ast(DEL_ITEM_IDX_OP, ret, expr2, state->source_name, state->source_line);
...
lang_ast_t *logic_xor(lang_state_t *state)
{
    ...
    if (lang_accept(state, TOKEN_XOR)) {
        ...
        ret = make_lang_ast(XOR_OP, expr1, expr2, state->source_name, state->source_line);
    }
    return ret;
}
...
lang_ast_t *statement(lang_state_t *state)
{
    ...
    if (lang_accept(state, TOKEN_lang)) {
        ...
        lang_expect(state, TOKEN_lang);
        return make_lang_ast(CONTINUE_STATEMENT, NULL, NULL, state->source_name, state->source_line);
...
//    lang_ast_t *func_body;
    ...
    global_env[get_std_thread_id()].function_name = func_symbol->symbol->name;
...
{
    std_lock_free_key_hash_value_callback_destroy(global_env[get_std_thread_id()].global_parse_error_need_clean_lang_ast_hash, &callback_destroy_lang_ast, NULL);
    std_lock_free_key_hash_value_callback_destroy(global_env[get_std_thread_id()].global_symbol_hash, &callback_destroy_lang_symbol, NULL);
...
std_rv_t lang_parse(lang_state_t *state, IN const std_char_t *file_name, IN std_char_t *source_buffer, IN std_int_t source_buffer_len)
{
    ...
    global_env[get_std_thread_id()].p_error_jump_buf = &state->error_jump_buf;
